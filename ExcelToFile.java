...
public class ExcelToFile {
	...
	public static void main(String[] args) {
		...
		String excelFileWithDataFullPath = "C:\\Users\\swartwba\\OneDrive - Rose-Hulman Institute of Technology\\Desktop\\Freshman Year\\Y1Q3\\ResearchProject\\HW1InfoTest.xlsx";
		//change the last \\ part to the desired file name and choose where to place the file
		String newFileWithInfoFullPath = "C:\\Users\\swartwba\\OneDrive - Rose-Hulman Institute of Technology\\Desktop\\Freshman Year\\Y1Q3\\ResearchProject\\HW1Info.txt";
		...
		try {
			...
			while (itr.hasNext()) {
				...
				while (cellIterator.hasNext()) {
					...
					if (currColumn == 3 || currColumn == 7 || currColumn == 16 || currColumn == 26) {
	 ...
	 * @paramchosenLabelForQuestions - the label for all the blocks in prarire learn, needs to be the same in each question
	 * 
	...
	private static String parseSubmission(String stringCellValue, String chosenLabelForQuestions) {
		...
		if(!stringCellValue.contains("[]"))
		{
			...
			for (int i = 0; i < arr.length(); i++) {
				String order = arr.getJSONObject(i).getString("tag");
				subAnswer += " " + order;
			}
		}
		return subAnswer;
